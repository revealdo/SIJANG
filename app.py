...
st.markdown("""
    <style>
    /* Membuat sidebar punya lebar tetap */
    [data-testid="stSidebar"] {
        min-width: 260px !important;
        max-width: 260px !important;
    }

    /* Membuat semua tombol menu lebarnya sama */
    [data-testid="stSidebar"] button {
        width: 100% !important;       /* tombol memenuhi sidebar */
        display: block !important;    /* memastikan full block */
        text-align: left !important;  /* teks rata kiri */
        padding: 8px 14px !important; /* biar rapi */
...
def export_laba_rugi_to_excel(df_lr):
    buffer = BytesIO()
    with pd.ExcelWriter(buffer, engine="openpyxl") as writer:
        df_lr.to_excel(writer, index=False, sheet_name="Laporan Laba Rugi")
    return buffer
...
def sidebar_menu():
    ...
    with st.sidebar:
        ...
        # Menu buttons (styled)
        menu_items = ["Dashboard", "Input Transaksi", "Jurnal Umum", "Buku Besar", "Neraca", "BP Utang", "BP Piutang", "Inventory","Laporan Laba Rugi"]
...
def input_transaksi_page():
    st.subheader("Input Saldo Awal 📘")

    with st.form("form_saldo_awal"):
        tanggal_sa = st.date_input("Tanggal", date.today(), key="tanggal_sa")
        deskripsi_sa = st.text_input("Deskripsi Saldo Awal", key="deskripsi_sa")

        daftar_akun_saldo_awal = [
            "Kas", "Piutang Usaha", "Utang Usaha", "Modal disetor",
        ...
        akun_sa = st.selectbox("Akun Saldo Awal", daftar_akun_saldo_awal, key="akun_sa")
        posisi = st.radio("Posisi", ["Debit", "Kredit"], horizontal=True)

        nominal_sa = st.number_input("Nominal Saldo Awal", min_value=0.0, format="%.2f", key="nominal_sa")

        simpan_sa = st.form_submit_button("Simpan Saldo Awal")
    ...
    if simpan_sa:
        ...
        if posisi == "Debit":
            debit_akun = akun_sa
            kredit_akun = "Saldo Awal"
        else:
            debit_akun = "Saldo Awal"
            kredit_akun = akun_sa

        new_sa = {
            "tanggal": tanggal_sa.strftime("%Y-%m-%d"),
            "deskripsi": deskripsi_sa if deskripsi_sa.strip() else f"Saldo awal {akun_sa}",
            "debit_akun": debit_akun,
            "kredit_akun": kredit_akun,
            "nilai": float(nominal_sa),
            "jenis_transaksi": "Saldo Awal",
            "nama_toko": "",
            "user": st.session_state.get("username", "unknown"),
        }

        st.session_state["jurnal_data"].append(new_sa)
    ...
    # ============================================================
    # 🟢 2. FORM INPUT TRANSAKSI JURNAL UMUM
    # ============================================================
    st.subheader("Input Transaksi Jurnal Umum")
...
# Ambil semua akun KECUALI "Saldo Awal"
    akun_list = sorted(
    a for a in set(df['debit_akun']).union(set(df['kredit_akun']))
    if a.lower() != "saldo awal"
    )
...
def laporan_laba_rugi_page():
    ...
    #   1. HITUNG PENDAPATAN
    # ============================
    # Pendapatan dianggap terjadi jika:
    #   Debit  = Kas
    #   Kredit = Pendapatan (akun yang mengandung "Penjualan")
    pendapatan = df[
        (df["debit_akun"].str.contains("Kas", case=False)) &
        (df["kredit_akun"].str.contains("Penjualan", case=False))
    ]["nilai"].sum()
    ...
    # ============================
    # Akun HPP adalah transaksi yang memakai akun "HPP" di debit ataupun kredit
    df_hpp = df[
    ...
    # Tambahkan tombol download (TIDAK mengubah tampilan)
    # ============================
#  DATAFRAME UNTUK DI-DOWNLOAD
# ============================
    df_lr = pd.DataFrame({
    "Kategori": [
        "Pendapatan",
...
# ============================
    buffer = export_laba_rugi_to_excel(df_lr)

    st.download_button(
    label="📥 Download Laporan Laba Rugi (Excel)",
    data=buffer,
...
def build_inventory_average(records):
    ...
    for r in records:
        ...
        rows.append({
            ...
            " Harga/Unit": rp(harga_unit),
            "HPP": rp(hpp) if hpp != "" else "",
            "Saldo Rp": rp(saldo_rp)
